...
// TestDstUnreachableErrnos pins the cross-platform "nothing is listening"
// classification (invented vs Winsock errno shapes, and the bare-timeout
// negative); the production comment explains why the numbers are matched
// directly.
...
// + light panel); stopLightPanel below exists from this task so the wiring
// compiles here.
...
The systray message loop cannot be automated from WSL; after `deploy\deploy.cmd` on Windows, run this sequence IN ORDER. Every step states its preconditions — a fresh daemon legitimately reports `unknown` until the first mute command or device event, so the middle steps establish state explicitly:
...
2. **Establish known state:** `mutastic.exe status` — if it says `unknown`, tap the mic's physical button once and re-check.
...
4. **Daemon-loss truthfulness:** from a console run `mutastic.exe shutdown` (prints `shutting down`). The tray keeps the RED muted icon; only the header/tooltip change to `Mutastic — daemon unreachable` and the action items gray out. It must NOT turn white.
...
9. **Quit stops everything:** menu **Quit** — `mutastic.exe status` prints `error: no daemon reachable` (daemon stopped), `curl -s http://127.0.0.1:42815/api/health` fails with connection refused (light panel stopped), AND the icon disappears (tray exited). One click, everything down.
