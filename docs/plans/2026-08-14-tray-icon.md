...
16. **Submenu rebuild across name-set changes:** in the panel's **Saved settings** card save two more names (`smoke b`, then `smoke c`) and overwrite `smoke test` with a fresh **Save** under the same name, and delete `smoke c` with its **Delete** button (the store caps at 100 names; overwrite and delete are the name-set changes under test — rename stays out of scope; whole-file reset coverage remains in step 17 via renaming the store file aside and back). Open the tray **Saved settings** submenu 5 times spread across these changes: every open renders exactly the current names, in CREATION order — the order the names were first saved in this tray process (`smoke b` after `smoke test`, `smoke c` last, so after the deletion the list reads `smoke test`, `smoke b`), NOT alphabetically (the tray keeps creation order by design so a menu row is permanently bound to one name and a click can never apply a different setting than the row displays; the web UI sorts alphabetically), with no duplicates and no phantom items left from earlier name sets, and clicking any listed name applies it.
