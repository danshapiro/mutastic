...
1. Imports: add `"context"` and `"sync"` to the import block (time, bytes, httptest etc. already present where needed).
...
Quit becomes the full cascade with confirmation: daemon `shutdown` (UDP, transport failure = unreachable = already the goal state; any other reply but `shutting down` = live refusal), panel `/api/shutdown` (HTTP from Task 4, via Task 2's `stopLightPanel`: `ECONNREFUSED` proves the panel is gone, everything else is a live-but-wedged failure), then the tray exits ONLY when every stop is confirmed or already absent — on a refusal it logs at ERROR, refreshes the display, and stays so the next Quit retries. Documentation catches up to the new semantics.
...
```markdown
  **Light preset**, **Light panel…**, and **Quit** — Quit stops everything
  ...
  be reached count as already stopped).
```
