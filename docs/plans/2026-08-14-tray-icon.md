...
3. Create `tray_windows.go` (logging, async click handlers, serialized light commands, and the serial-queue-safe budgets come from load-bearing findings LB-3/LB-4/LB-5; keep-last-icon, the mute-everything handler wiring, the neutral initial icon, JSONL logging with the stdlib bridge, and click-ordered light sends come from plan-review rounds 1-3):
...
Quit becomes the full cascade: daemon `shutdown` (UDP), panel `/api/shutdown` (HTTP from Task 4), then the tray exits — all best-effort, any failure still quits the tray. A transport failure reaching the panel means "no panel" (dead or never started), which is already the goal state of Quit, so only an alive-but-refusing panel is an error. Documentation catches up to the new semantics.

**Files:**
- Modify: `traystate.go` (`trayActions.stopPanel` field, `onQuit`, new `stopLightPanel`, imports)
- Modify: `traystate_test.go` (spy + Quit expectations, new `stopLightPanel` tests)
- Modify: `tray_windows.go` (wire `stopPanel`, Quit tooltip text)
...
**Interfaces:**
- Consumes: Task 4's `POST /api/shutdown`; `trayPanelURL` const (tray_windows.go); existing `trayActions`/`errString`.
- Produces: `stopLightPanel(baseURL string) error`; `trayActions.stopPanel func() error` (wired in tray_windows.go); Quit semantics "stops everything mutastic runs".

- [ ] **Step 1: Write the failing behavioral tests**

In `traystate_test.go` (its import block gains `"net"`, `"net/http"`, and `"net/http/httptest"` for the new tests):
...
Run: `go test . -run 'TestTrayQuit|TestStopLightPanel' -count=1`

Expected: FAIL to BUILD — `trayActions.stopPanel` and `stopLightPanel` are undefined (the intended missing behavior).

- [ ] **Step 3: Add the minimal production implementation**

1. In `traystate.go`: imports add `"fmt"`, `"net/http"`, `"time"`; struct gains the field:
...
2. In `tray_windows.go`: add the wiring to the `actions` literal and update the Quit tooltip:
