...
9. **Quit:** menu **Quit** — `mutastic.exe status` prints `error: no daemon reachable` (daemon stopped) AND the icon disappears. The `mutastic ui` HTTP server process intentionally stays up (no shutdown path by design; harmless — it has no backend without the daemon and everything restarts together at next login/deploy).
