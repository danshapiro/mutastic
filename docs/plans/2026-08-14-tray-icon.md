...
// endpoint; the tray's Quit cascade adopts it in Task 5). ECONNREFUSED
// proves nothing is listening - dead or never started, which is the goal
// state of any stop request - so only an ALIVE panel (a non-OK status, or
// a wedged listener that never answers) is an error worth surfacing.
func stopLightPanel(baseURL string) error {
	...
	if err != nil {
		// ECONNREFUSED proves nothing is listening: the panel is gone. Any
		// other transport failure (timeout, reset) may be a live but wedged
		// panel - that is a real error. (errors.Is reaches WSAECONNREFUSED
		// on Windows through the net package's errno mapping.)
...
1. Imports: add `"context"` to the import block (time, bytes, httptest etc. already present where needed).

2. `uiServer` struct, new field:
...
3. Route in `ServeHTTP`, after the `"/api/group"` case:
...
4. `runUI` wiring (replace the server construction):
...
Quit becomes the full cascade: daemon `shutdown` (UDP), panel `/api/shutdown` (HTTP from Task 4), then the tray exits — all best-effort, any failure still quits the tray. Only `ECONNREFUSED` proves the panel is gone (nothing is listening — Quit's goal state); every other transport failure (timeout, reset) can come from a live, wedged panel and is a real error, ERROR-logged by `onQuit`. Documentation catches up to the new semantics.
...
1. Extend `traySpy` with a panel recorder and error:
...
and in its `actions()` add:

```go
		stopPanel:     func() error { s.calls = append(s.calls, "panelstop"); return s.stopErr },
```

2. Replace `TestTrayQuitStopsDaemonThenQuits` with the cascade expectations:
