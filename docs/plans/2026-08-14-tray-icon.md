...
2. **Establish known state:** `C:\Users\dan\code\mutastic-deploy\mutastic.exe status` — if it says `unknown`, tap the mic's physical button once and re-check. (Until the state is known, the menu's **Muted** item stays gray by design — the tray never guesses the mic state.)
3. **Live→muted display:** press the physical mic button (or the Stream Deck mute key) so the mic ends MUTED: within ~2 s the icon is red, the header reads `Mutastic — muted`, the **Muted** item is checked. (If the mic ended unmuted, press once more.)
...
7. **Mute-everything from the menu:** with a meeting app open click **Muted**: the mic AND the meeting app change together (deck-key flow); click again and both change back. Direction depends on the starting state — the invariant is that they move TOGETHER.
