...
- The saved-settings list itself stays uncapped by design (YAGNI — a settings menu that long is unusable anyway; no `settings list` chunking verb is planned), but the read side must always hold it: the root-package daemon-reply read buffer (`askDaemon` in deckplugin.go — the single read path shared by the CLI, web UI, and tray) is raised from 2048 to 8192 bytes in Task 2 (UDP datagrams reach ~64 KB, so headroom exists; 8192 bytes holds ~185 newline-joined names even at the 43-byte cap), while the deckplugin's separate OpenDeck socket is unchanged because it never reads settings lists (F2).
...
**Interfaces:**
- Consumes: `Registry.Resolve`/`Registry.NameFor` (names.go); per-Manager `state.Status()`/`state.TargetOn()` (state.go); `Manager.HandleCommand` (manager.go); fleet helpers `callLight` + `label` and the `handleAll` fan-out pattern (multi.go); `ByteToKelvin` (frame.go); the `Registry.saveLocked` persistence conventions (`json.Marshal` + `os.MkdirAll` + 0o644, tolerate-corrupt load) — but NOT its non-atomic `os.WriteFile`: the store write is atomic (Behavior 1, F3).
...
func (s *SettingsStore) Save(name string, snap SavedSetting) error // overwrite by exact name; error = persistence failure, store left unchanged
...
- Produces (wire verbs, contract §3 — literal replies):
  - `settings save <name>` → `saved "<name>" (N lights)` (N = SAVED known-state lights); failures `error: settings save failed: <err>` (persistence), `error: no known light state to save`
...
- `TestSavedSettingsStorePersistedAcrossReload` — a second `NewSettingsStore` on the same path sees the snapshot (this IS the pinned reload-after-successful-save scenario, F3); a corrupt file silently lists empty; `NewSettingsStore("")` is disabled and a no-op.
...
3. Reply buffer (F2): `askDaemon`'s reply read buffer is raised from 2048 to 8192 bytes (deckplugin.go — the single root-package read path shared by the CLI, web UI, and tray; UDP datagrams reach ~64 KB, so headroom exists; 8192 bytes holds ~185 newline-joined names even at the 43-byte cap): the settings list itself stays uncapped, and no saved name ever silently truncates out of the tray menu or the web UI; the deckplugin's separate OpenDeck socket never reads settings lists and is unchanged.
...
5. README protocol/CLI docs enumerate the three verbs, the name rules and BOTH validation errors (`error: invalid settings name` for empty/newline/`error:`-prefixed names — case-insensitive; `error: settings name too long (max 43 bytes)` past the byte cap, with the 64-byte-receive-buffer rationale), the `saved "<name>" (N lights)`/sorted-list/empty-reply-is-none shapes, the apply fan-out line shape with `unreachable, skipped` per-key errors, `error: unknown setting "<name>"`, `error: no lights connected`, the save failures `error: no known light state to save` and `error: settings save failed: <err>`, persistence in `%LOCALAPPDATA%\mutastic\light-settings.json`, quoted multi-word names (`mutastic.exe light settings save "movie mode"`), and the deliberate no-delete/rename-verb (YAGNI) note.
...
**Commit:**
`git add internal/daemon/daemon.go internal/daemon/daemon_test.go deckplugin.go deckplugin_test.go README.md && git commit -m "daemon: dedupe settings-list polls; loopback coverage + README for light settings verbs; askDaemon reply buffer 2048→8192 (uncapped settings list)"`
...
4. POST maps to HTTP 502 `{"names":[],"error":"<verbatim daemon reply|transport error>"}` ONLY on a transport error OR on a reply that is EXACTLY ONE line starting with `error:` (whole-reply errors: unknown setting, invalid name, persistence disabled, too-long name, no known light state to save, settings save failed); a MULTI-LINE apply reply whose first line is an inline `error: light ...` skip is PARTIAL SUCCESS — the resolvable lights did apply — and answers HTTP 200 + refreshed names (LB-4 ruling).
