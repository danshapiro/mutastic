...
| `group-*-output` "Mixed" literal pins | **Replaced by `setChip`.** The two Mixed writes are now `setChip($("group-brightness-output"), "Mixed", true);` (index.html:853) and `setChip($("group-temp-output"), "Mixed", true);` (index.html:863) — the old literal `$("group-*-output").textContent = "Mixed";` matches NOTHING on the final page. | Re-pin to the `setChip(..., "Mixed", true)` call shape (this is the stale `TestEmbeddedLightUICardsUseTheirOwnIdentityAsTarget` fragment, which fails until the suite refresh lands); the trailing `true` also asserts the dim tone (row below). |
| Mic click → immediate state assertions | **175 ms commit buffer added.** Every mic-button click arms `micCommittedAt = Date.now()` (index.html:1063); `updateMic` drops poll-driven state writes younger than 175 ms (index.html:1023) so a mid-flight poll cannot revert a just-pressed verb before the daemon acks. | After a mic click, exercise the POLL path (drive `updateMic` through a poll response after the 175 ms window, or fake `Date.now` in the stub) — do not assert the immediate state from the POST response. |
| Poll-during-drag guards | **Guard widened.** While a slider drag is active (`state.active` key set OR the input focused), a poll re-render writes NEITHER the input value, NOR the readout chip (`textContent`/`data-tone`), NOR the track fill to that control (per-card: index.html:808-819 incl. the fill write at :819; group sliders: same pattern in `updateGroupDefaults`). | Stub tests hold a drag active across a poll and assert value + chip + fill all stay user-owned; after release/blur, the next poll re-syncs all three. |
| Unknown/Mixed chip dimming | **New `data-tone="unknown"` hook.** `setChip(output, text, dim)` (index.html:831) stamps/deletes `data-tone="unknown"`; CSS dims `.control-label output[data-tone="unknown"]` to `--text-3` (index.html:240); both group outputs ship the attribute in the initial markup (index.html:618, :626). | Pin the ATTRIBUTE (and its removal on definite values), not color literals. |

## Behavioral deltas (adjudicated)

- **Error aura (whole-branch review Finding 3, 2026-08-17): design docs govern.** `applyGlow` computes `const on = lightIsOn(light) && !light.error;` (index.html:997) — an errored light carries ZERO card aura (`--glow-bri`/`--glow-hue` → 0); the card-error strip + rose border own the error state. This supersedes the plan's old "aura present but outranked" Task-7 Step-4 wording (plan corrected in the same wave).
