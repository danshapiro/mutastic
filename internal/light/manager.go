...
// HandleCommandWithTimeout calls HandleCommand with a context that times out
// if the light doesn't respond quickly. Used by MultiManager fan-outs where
// one wedged light must never block the fleet. Returns "error: timeout" on
// context cancellation.
func (m *Manager) HandleCommandWithTimeout(ctx context.Context, cmd string) string {
	type result struct {
		reply string
	}
	done := make(chan result, 1)
	go func() {
		done <- result{m.HandleCommand(cmd)}
	}()
...
// ProbeStatus returns a status string, taking m.mu lock to verify the port is
// still connected. Returns "unknown" if disconnected, or a timeout wrapped
// error if the context expires (e.g., a wedged write is holding the lock).
func (m *Manager) ProbeStatus(ctx context.Context) string {
