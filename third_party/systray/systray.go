...
type MenuItem struct {
	// click is the user's click handler, invoked by the native dispatch
	// loops via systrayMenuItemSelected. VENDORED DIVERGENCE (see
	// PATCHES.md): upstream stored this as a bare func field; mutastic
	// rebinds handlers AFTER the message pump is already dispatching
	// (menu rebuilds), so the field is an atomic.Pointer with a paired
	// setter (Click) and getter (getClick) - a post-pump (re)binding can
	// never be observed half-written by the dispatch side.
	click atomic.Pointer[func()]
...
func (item *MenuItem) Click(fn func()) {
	// VENDORED (PATCHES.md): atomic store - safe to call after the native
	// message pump has started dispatching clicks.
	item.click.Store(&fn)
}

// getClick loads the current click handler (nil when none was ever
// bound). VENDORED (PATCHES.md): paired with Click's atomic store, so the
// native dispatch below never reads a torn binding.
func (item *MenuItem) getClick() func() {
	if fn := item.click.Load(); fn != nil {
		return *fn
	}
	return nil
}
...
func systrayMenuItemSelected(id uint32) {
	...
	// VENDORED (PATCHES.md): synchronized load of the click binding.
	if fn := item.getClick(); fn != nil {
		fn()
	}
}
