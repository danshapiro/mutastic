...
var (
	// VENDORED DIVERGENCE (see PATCHES.md): like MenuItem.click, these
	// are written through setters callable after the native dispatch is
	// running and read by that dispatch, so they are synchronized.
...
func setOnClick(fn func(menu IMenu)) {
	enableOnClick()
	onClick.Store(&fn) // VENDORED (PATCHES.md): synchronized binding
}

func setOnDClick(fn func(menu IMenu)) {
	enableOnClick()
	onDClick.Store(&fn) // VENDORED (PATCHES.md): synchronized binding
}

func setOnRClick(fn func(menu IMenu)) {
	enableOnClick()
	onRClick.Store(&fn) // VENDORED (PATCHES.md): synchronized binding
}

// VENDORED (PATCHES.md): synchronized readers for the callbacks above,
// so the native click exports never observe a half-written binding.
func getOnClick() func(menu IMenu) {
	if fn := onClick.Load(); fn != nil {
		return *fn
	}
	return nil
}

func getOnDClick() func(menu IMenu) {
	if fn := onDClick.Load(); fn != nil {
		return *fn
	}
	return nil
}

func getOnRClick() func(menu IMenu) {
	if fn := onRClick.Load(); fn != nil {
		return *fn
	}
	return nil
}
...
func systray_on_click() {
	if dClickTime == 0 {
		...
		if nowMilli-dClickTime < dClickTimeMinInterval {
			...
			if fn := getOnDClick(); fn != nil { /* VENDORED (PATCHES.md): synchronized load */
				fn(st)
				return
			}
		} else {
	...
	if fn := getOnClick(); fn != nil { /* VENDORED (PATCHES.md): synchronized load */
		fn(st)
	}
}
...
func systray_on_rclick() {
	if fn := getOnRClick(); fn != nil { /* VENDORED (PATCHES.md): synchronized load */
		fn(st)
	} else {
		C.show_menu()
	}
