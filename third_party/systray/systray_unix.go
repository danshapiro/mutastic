...
type UnimplementedStatusNotifierItem struct {
	...
	// VENDORED DIVERGENCE (see PATCHES.md): activate/dActivate are written
	// through setOnClick/setOnDClick (callable after the dbus handler is
	// live) and read by the dbus-exported Activate handler below, so they
	// are synchronized exactly like MenuItem.click.
	activate  atomic.Pointer[func(x int32, y int32)]
	dActivate atomic.Pointer[func(x int32, y int32)]
...
func (m *UnimplementedStatusNotifierItem) Activate(x int32, y int32) (err *dbus.Error) {
	if m.dActivateTime == 0 {
		...
		if nowMilli-m.dActivateTime < dClickTimeMinInterval {
			...
			// VENDORED (PATCHES.md): synchronized load.
			if fn := m.dActivate.Load(); fn != nil {
				(*fn)(x, y)
				return
			}
		} else {
...
func setOnClick(fn func(menu IMenu)) {
	// VENDORED (PATCHES.md): synchronized binding.
	wrapped := func(x int32, y int32) { fn(nil) }
	usni.activate.Store(&wrapped)
}

func setOnDClick(fn func(menu IMenu)) {
	// VENDORED (PATCHES.md): synchronized binding.
	wrapped := func(x int32, y int32) { fn(nil) }
	usni.dActivate.Store(&wrapped)
}
