...
type winTray struct {
	...
	// VENDORED DIVERGENCE (see PATCHES.md): the icon activation callbacks
	// share the shape that was flagged for MenuItem.click - written through
	// setters callable after the message pump is running, read by the pump
	// (wndProc below) - so they are synchronized exactly like it.
	onClick  atomic.Pointer[func(menu IMenu)]
	onDClick atomic.Pointer[func(menu IMenu)]
	onRClick atomic.Pointer[func(menu IMenu)]
}

// VENDORED (PATCHES.md): synchronized readers for the callbacks above,
// so the pump never observes a half-written binding.
func (t *winTray) getOnClick() func(menu IMenu) {
	if fn := t.onClick.Load(); fn != nil {
		return *fn
	}
	return nil
}

func (t *winTray) getOnDClick() func(menu IMenu) {
	if fn := t.onDClick.Load(); fn != nil {
		return *fn
	}
	return nil
}

func (t *winTray) getOnRClick() func(menu IMenu) {
	if fn := t.onRClick.Load(); fn != nil {
		return *fn
	}
	return nil
}
...
func (t *winTray) setOnClick(fn func(menu IMenu)) {
	t.onClick.Store(&fn) // VENDORED (PATCHES.md): synchronized binding
}

func (t *winTray) setOnDClick(fn func(menu IMenu)) {
	t.onDClick.Store(&fn) // VENDORED (PATCHES.md): synchronized binding
}

func (t *winTray) setOnRClick(fn func(menu IMenu)) {
	t.onRClick.Store(&fn) // VENDORED (PATCHES.md): synchronized binding
}
...
func (t *winTray) wndProc(hWnd Handle, message uint32, wParam, lParam uintptr) (lResult uintptr) {
	...
	case t.wmSystrayMessage:
		switch lParam {
		// VENDORED (PATCHES.md): synchronized callback loads.
		case WM_RBUTTONUP:
			if fn := t.getOnRClick(); fn != nil {
				fn(t)
			} else {
				t.ShowMenu()
			}
		case WM_LBUTTONUP:
			if fn := t.getOnClick(); fn != nil {
				fn(t)
			}
		case WM_LBUTTONDBLCLK:
			if fn := t.getOnDClick(); fn != nil {
				fn(t)
			}
		}
